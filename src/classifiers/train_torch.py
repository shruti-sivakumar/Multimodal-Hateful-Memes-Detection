...
def tune_threshold(y_true: np.ndarray, probs: np.ndarray, metric: str = "f1") -> Tuple[float, float]:
    """
    Tune threshold on dev probabilities.
    metric: "f1" or "acc"
...
class TokenDataset(Dataset):
    """
    For token-based models (CNN / Transformer joint tokens).
...
class CrossTokenDataset(Dataset):
    """
    For cross-attention model: separate text + image tokens.
    text_tokens:  float32 [N, Lt, Dt]
    image_tokens: float32 [N, Li, Di]
    text_mask:    int64   [N, Lt] (1=valid, 0=pad) or None
    image_mask:   int64   [N, Li] (1=valid, 0=pad) or None
    y:            float32 [N]
    """
    def __init__(
        self,
...
def train_embedding_model(
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        # Eval
        dev_probs, dev_y = eval_embedding_model(model, dev_loader, device)

        # Tune threshold on dev
        t, _ = tune_threshold(dev_y, dev_probs, metric=cfg.threshold_metric)
        metrics = compute_metrics(dev_y, dev_probs, threshold=t)

        print(f"[Epoch {epoch}] train_loss={avg_loss:.4f}  dev_auc={metrics['auc']:.4f}  dev_acc={metrics['acc']:.4f}  dev_f1={metrics['f1']:.4f}  thr={t:.2f}")
    ...
    # Final best metrics
    dev_probs, dev_y = eval_embedding_model(model, dev_loader, device)
    final_metrics = compute_metrics(dev_y, dev_probs, threshold=best_threshold)
    final_metrics["best_threshold"] = float(best_threshold)
    final_metrics["pos_weight"] = float(pw)
...
def train_token_model(
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        for xb, yb, mb in train_loader:
            ...
            # Support models that accept (x, mask) or only (x)
            try:
        ...
        metrics = compute_metrics(dev_y_np, dev_probs, threshold=t)
    ...
    dev_probs, dev_y_np = eval_token_model(model, dev_loader, device)
    final_metrics = compute_metrics(dev_y_np, dev_probs, threshold=best_threshold)
    final_metrics["best_threshold"] = float(best_threshold)
    final_metrics["pos_weight"] = float(pw)
    return final_metrics
